...
router.post('/signup', async function (req, res) {
  ...
  var now = new Date().getTime(); // milliseconds
    ...
    var newCust = { username: username, password: password, name: name, phone: phone, email: email, active: 1, token: token };
    var newID = await CustomerDAO.insert(newCust);
    var result = await EmailUtil.send(email);
    
   
  if (dbCust) {
    MyUtil.showAlertAndRedirect(res, 'Email hoặc tài khoản đã tồn tại', './signup');
  } 
  if (newID) {
    MyUtil.showAlertAndRedirect(res, 'Kiểm tra email nhé!', './login');
  }
...
router.get('/verify', async function (req, res) {
  ...
  MyUtil.showAlertAndRedirect(res, 'Thành công!', './login');
  // if (result) {
